...
#Region "ExecuteNonQuery"
    ...
    'Execute mot SqlCommand su dung SqlConnection, sau do giai phong command
    ...
    Public Overloads Shared Function ExecuteNonQuery(ByVal connection As SqlConnection, ByVal command As SqlCommand, ByVal data As DataTable)
        If (connection Is Nothing) Then Throw New ArgumentNullException("connection")
        If (command Is Nothing) Then Throw New ArgumentNullException("command")
                ...
                For Each param In command.Parameters
                    ...
                    If param.Direction = ParameterDirection.Input Or param.Direction = ParameterDirection.InputOutput Then
                        ...
                        If (param.SourceColumn = Nothing) OrElse (param.SourceColumn = String.Empty) Then
                            ...
                            'Do nothing!! (Cho là đã được set gia trị trước)
                        Else
                            'Add value
...
#Region "ExecuteScalar"
    ...
    'cmd: Command object để execute (phải tạo sẵn và map SourceColumn với dt)
    'dt: DataTable object chứa dữ liệu
    Public Overloads Shared Function ExecuteScalar(ByVal connection As SqlConnection, ByVal command As SqlCommand, ByVal data As DataTable) As Object
        If (connection Is Nothing) Then Throw New ArgumentNullException("connection")
        If (command Is Nothing) Then Throw New ArgumentNullException("command")
        If (data Is Nothing) Then Throw New ArgumentNullException("data")
        Dim dr As DataRow
...
#Region "FillDatatable"
    ...
    ' using the provided parameters.
    ' e.g.:  
    '   FillDatatable (connString, CommandType.StoredProcedure, "GetOrders", ds, new String() = {"orders"}, new SqlParameter("@prodid", 24))
    ' Parameters:    
    ' -connectionString: A valid connection string for a SQLConnection
    ' -commandType: the CommandType (stored procedure, text, etc.)
    ' -commandText: the stored procedure name or T-Oracle command
    ...
    ' -tableNames: this array will be used to create table mappings allowing the DataTables to be referenced
    ' by a user defined name (probably the actual table name)
    Public Overloads Shared Sub FillDatatable(ByVal connection As SqlConnection, ByVal commandType As CommandType, _
    ...
    ' using the provided parameters.
    ' e.g.:  
    '   FillDatatable (conn, CommandType.StoredProcedure, "GetOrders", ds, new String() {"orders"}, new SqlParameter("@prodid", 24))
    ' Parameters:
    ' -connection: A valid SQLConnection
    ' -commandType: the CommandType (stored procedure, text, etc.)
    ' -commandText: the stored procedure name or T-Oracle command
    ...
    ' -tableNames: this array will be used to create table mappings allowing the DataTables to be referenced
    '             by a user defined name (probably the actual table name)
    Public Overloads Shared Sub FillDatatable(ByVal transaction As SqlTransaction, ByVal commandType As CommandType, _
     ByVal commandText As String, ByVal dataTable As DataTable)
    ...
    ' using the provided parameters.
    ' e.g.:  
    '   FillDatatable(trans, CommandType.StoredProcedure, "GetOrders", ds, new string() {"orders"}, new SqlParameter("@prodid", 24))
    ' Parameters:
    ' -transaction: A valid OracleTransaction
    ' -commandType: the CommandType (stored procedure, text, etc.)
    ' -commandText: the stored procedure name or T-Oracle command
    ...
    ' Private helper method that execute a SQLCommand (that returns a resultset) against the specified OracleTransaction and SQLConnection
    ' using the provided parameters.
    ' e.g.:  
    '   FillDatatable(conn, trans, CommandType.StoredProcedure, "GetOrders", ds, new String() {"orders"}, new SqlParameter("@prodid", 24))
    ' Parameters:
    ' -connection: A valid SQLConnection
    ' -transaction: A valid OracleTransaction
    ' -commandType: the CommandType (stored procedure, text, etc.)
    ...
    '             by a user defined name (probably the actual table name)
    ' -commandParameters: An array of OracleParamters used to execute the command
    Private Overloads Shared Sub FillDatatable(ByVal connection As SqlConnection, ByVal transaction As SqlTransaction, ByVal commandType As CommandType, _
     ByVal commandText As String, ByVal dataTable As DataTable, _
     ByVal ParamArray commandParameters() As SqlParameter)
...
#Region "Connection"
    ...
    Public Shared Function OpenConnection(ByVal ConnectionString As String) As SqlConnection
...
#Region "Parameter"
    Public Shared Function CreateParameter(ByVal parameterName As String, ByVal type As SqlDbType, ByVal value As Object) As SqlParameter
    ...
    Public Shared Function CreateParameter(ByVal parameterName As String, ByVal type As SqlDbType, ByVal value As Object, ByVal direction As ParameterDirection) As SqlParameter
        Dim param As New SqlParameter(parameterName, type)
        param.Value = value
    ...
    Public Shared Function CreateParameter(ByVal parameterName As String, ByVal type As SqlDbType, ByVal size As Integer, ByVal value As Object, ByVal direction As ParameterDirection) As SqlParameter
